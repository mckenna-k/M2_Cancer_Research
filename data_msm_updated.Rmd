...
df_simu1 <- cbind("Patient"=as.character(20001:21000),process(n=1000,0.3,0.3,0.3,0.3,0.3,97),"stage"=stage_1,"var_nulle"=sample(0:1,1000,replace = TRUE),"gender"=rep(NA,1000),"age"=rep(NA,1000),"type"=rep("SIMU1",1000))

stage_2 <- sample(c("I - II","III - IV"),1000,replace = TRUE,prob=c(0.2,0.8))
df_simu2 <- cbind("Patient"=as.character(21001:22000),process(n=100,0.3,0.6,0.6,0.2,0.2,97),"stage"=stage_2,"var_nulle"=sample(0:1,1000,replace = TRUE),"gender"=rep(NA,1000),"age"=rep(NA,1000),"type"=rep("SIMU2",1000))
...
for(kc in 
      c("BRCA", "OV", "PAAD", "LUAD", "LUSC", "COAD", "SIMU1", "SIMU2"
       )
    ) {
...
```{r}
data = sapply(unique(df_msm$type), function(kc) {
  Qmat <- matrix(c(1,1,1,0,1,1,0,0,0),ncol=3,byrow = TRUE)
