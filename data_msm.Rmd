...
# cox01_brca_stage = coxph(Surv(T01.time,T01)~stage, data = df_new[df_new$type == 'BRCA',])
# summary(cox01_brca_stage)$coefficients[2:4]
# 
# cox02_brca_stage = coxph(Surv(T02.time,T02)~stage, data = df_new[df_new$type == 'BRCA',])
# summary(cox02_brca_stage)$coefficients[2:4]
# 
# cox12_brca_stage = coxph(Surv(T12.time,T12)~stage, data = df_new[df_new$type == 'BRCA',])
# summary(cox12_brca_stage)$coefficients[2:4]
# 
# 
# cox01_ov_stage = coxph(Surv(T01.time,T01)~stage, data = df_new[df_new$type == 'OV',])
# summary(cox01_ov_stage)$coefficients[2:4]
# 
# cox02_ov_stage = coxph(Surv(T02.time,T02)~stage, data = df_new[df_new$type == 'OV',])
# summary(cox02_ov_stage)$coefficients[2:4]
# summary(cox02_ov_stage)$conf.int[c(1,3,4)]
# 
# cox12_ov_stage = coxph(Surv(T12.time,T12)~stage, data = df_new[df_new$type == 'OV',])
# sum = summary(cox12_ov_stage)


#HR des modeles de cox marginaux
